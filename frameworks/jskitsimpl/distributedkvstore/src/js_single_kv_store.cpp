...
static JSUtil::StatusMsg GetVariantArgs(napi_env env, size_t argc, napi_value* argv, VariantArgs& va)
{
    ...
    if (type == napi_string) {
        ...
        if (keyPrefix.empty()) {
            va.errMsg = "Parameter error:Parameters verification failed";
...
napi_value JsSingleKVStore::New(napi_env env, napi_callback_info info)
{
    ...
    ASSERT_ERR(env, kvStore != nullptr, Status::INVALID_ARGUMENT, "Parameter error:Parameters verification failed");
