...
public:
    static constexpr uint32_t SHORT_SYNC_INTERVAL = 100;
    static constexpr uint32_t LONG_SYNC_INTERVAL = 300;
...
HWTEST_F(TaskSchedulerTest, At, TestSize.Level0)
{
    ...
    auto expiredTime = std::chrono::system_clock::now() + std::chrono::milliseconds(SHORT_SYNC_INTERVAL);
    int testData = 10;
    auto blockData = std::make_shared<BlockData<int>>(LONG_SYNC_INTERVAL, testData);
    ...
    blockData->Clear();
    expiredTime = std::chrono::system_clock::now() + std::chrono::milliseconds(SHORT_SYNC_INTERVAL);
...
HWTEST_F(TaskSchedulerTest, Every, TestSize.Level0)
{
    ...
    auto blockData = std::make_shared<BlockData<int>>(LONG_SYNC_INTERVAL, 0);
    taskScheduler.Every(std::chrono::milliseconds(SHORT_SYNC_INTERVAL), [blockData]() {
        int testData = 1;
        blockData->SetValue(testData);
    });
    for (int i = 1; i < 5; ++i) {
...
HWTEST_F(TaskSchedulerTest, Reset1, TestSize.Level0)
{
    ...
    expiredTime += std::chrono::milliseconds(LONG_SYNC_INTERVAL);
    auto atTaskId2 = taskScheduler.At(expiredTime, []() {});

    auto resetTaskId1 = taskScheduler.Reset(atTaskId1, std::chrono::milliseconds(SHORT_SYNC_INTERVAL));
    ASSERT_EQ(resetTaskId1, atTaskId1);

    auto resetTaskId2 = taskScheduler.Reset(atTaskId2, std::chrono::milliseconds(SHORT_SYNC_INTERVAL));
...
HWTEST_F(TaskSchedulerTest, Reset2, TestSize.Level0)
{
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(SHORT_SYNC_INTERVAL));
