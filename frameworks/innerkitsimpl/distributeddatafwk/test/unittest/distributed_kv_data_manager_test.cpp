...
/**
 * @tc.name: DeleteKvStoreWithNewFunction
 * @tc.desc: Test delete kv store with new Delete function signature
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithNewFunction, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithNewFunction begin.");
    Options options = { .createIfNecessary = true, .encrypt = false, .autoSync = false };
    std::shared_ptr<SingleKvStore> kvStore;
    Status status = manager.GetSingleKvStore(options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);
    ASSERT_NE(kvStore, nullptr);

    status = kvStore->Put({"test_key"}, {"test_value"});
    ASSERT_EQ(status, Status::SUCCESS);

    status = manager.CloseAllKvStore({ "default_appId" });
    ASSERT_EQ(status, Status::SUCCESS);

    status = manager.DeleteKvStore("default_storeId");
    ASSERT_EQ(status, Status::SUCCESS);
}

/**
 * @tc.name: DeleteKvStoreWithBackupAndRestore
 * @tc.desc: Test delete kv store after backup and restore operations
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithBackupAndRestore, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithBackupAndRestore begin.");
    std::string storeId = "backup_restore_store";
    Options options = { .createIfNecessary = true, .encrypt = false, .autoSync = false };
    std::shared_ptr<SingleKvStore> kvStore;
    Status status = manager.GetSingleKvStore(storeId, options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);
    ASSERT_NE(kvStore, nullptr);

    status = kvStore->Put({"backup_key"}, {"backup_value"});
    ASSERT_EQ(status, Status::SUCCESS);

    std::string baseDir = "/data/service/el1/public/database/DeleteKvStoreTest";
    status = kvStore->Backup("test_backup", baseDir);
    ASSERT_EQ(status, Status::SUCCESS);
    ...
    options.createIfNecessary = true;
    status = manager.GetSingleKvStore(storeId, options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);

    status = kvStore->Restore("test_backup", baseDir);
    ASSERT_EQ(status, Status::SUCCESS);

    Value value;
    status = kvStore->Get({"backup_key"}, value);
    ASSERT_EQ(status, Status::SUCCESS);
    ASSERT_EQ(value.ToString(), "backup_value");
}

/**
 * @tc.name: DeleteKvStoreWithEncryption
 * @tc.desc: Test delete encrypted kv store with new function
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithEncryption, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithEncryption begin.");
    std::string storeId = "encrypted_delete_store";
    Options options = { .createIfNecessary = true, .encrypt = true, .autoSync = false };
    std::shared_ptr<SingleKvStore> kvStore;
    Status status = manager.GetSingleKvStore(storeId, options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);
    ASSERT_NE(kvStore, nullptr);

    status = kvStore->Put({"encrypted_key"}, {"encrypted_value"});
    ASSERT_EQ(status, Status::SUCCESS);
...
/**
 * @tc.name: DeleteKvStoreWithSubUser
 * @tc.desc: Test delete kv store with subUser parameter
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithSubUser, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithSubUser begin.");
    std::string storeId = "subuser_delete_store";
    Options options = { .createIfNecessary = true, .encrypt = false, .autoSync = false, .subUser = 100 };
    std::shared_ptr<SingleKvStore> kvStore;
    Status status = manager.GetSingleKvStore(storeId, options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);
    ASSERT_NE(kvStore, nullptr);

    status = kvStore->Put({"subuser_key"}, {"subuser_value"});
    ASSERT_EQ(status, Status::SUCCESS);
...
/**
 * @tc.name: DeleteKvStoreMultipleTimes
 * @tc.desc: Test delete multiple kv stores sequentially
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreMultipleTimes, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreMultipleTimes begin.");
    std::vector<std::string> storeIds = {"store1", "store2", "store3"};
    Options options = { .createIfNecessary = true, .encrypt = false, .autoSync = false };
    ...
    Status status = manager.CloseAllKvStore({ "default_appId" });
    ASSERT_EQ(status, Status::SUCCESS);

    for (const auto& storeId : storeIds) {
        Status status = manager.DeleteKvStore(storeId);
        ASSERT_EQ(status, Status::SUCCESS);
    }
}

/**
 * @tc.name: DeleteKvStoreWithCustomDir
 * @tc.desc: Test delete kv store with custom directory
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithCustomDir, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithCustomDir begin.");
    std::string storeId = "custom_dir_delete_store";
    std::string customDir = "/data/service/el1/public/database/CustomDirTest";
    Options options = { .createIfNecessary = true, .encrypt = false, .autoSync = false, .baseDir = customDir };
    std::shared_ptr<SingleKvStore> kvStore;
    Status status = manager.GetSingleKvStore(storeId, options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);
    ASSERT_NE(kvStore, nullptr);

    status = kvStore->Put({"custom_dir_key"}, {"custom_dir_value"});
    ASSERT_EQ(status, Status::SUCCESS);
...
/**
 * @tc.name: DeleteKvStoreWithInvalidStoreId
 * @tc.desc: Test delete kv store with invalid store ID
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithInvalidStoreId, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithInvalidStoreId begin.");
    std::string invalidStoreId = "non_existent_store";
    Status status = manager.DeleteKvStore(invalidStoreId);
    ASSERT_NE(status, Status::SUCCESS);
}

/**
 * @tc.name: DeleteKvStoreWithErrorReporting
 * @tc.desc: Test delete kv store error reporting functionality
 * @tc.type: FUNC
 */
HWTEST_F(DistributedKvDataManagerTest, DeleteKvStoreWithErrorReporting, TestSize.Level0)
{
    ZLOGI("DeleteKvStoreWithErrorReporting begin.");
    std::string storeId = "error_reporting_store";
    Options options = { .createIfNecessary = true, .encrypt = false, .autoSync = false };
    std::shared_ptr<SingleKvStore> kvStore;
    Status status = manager.GetSingleKvStore(storeId, options, kvStore);
    ASSERT_EQ(status, Status::SUCCESS);
    ...
    status = kvStore->Put({"error_key"}, {"error_value"});
    ASSERT_EQ(status, Status::SUCCESS);
