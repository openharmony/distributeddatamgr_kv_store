/*
 * Copyright (c) 2022 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include <condition_variable>
#include <gtest/gtest.h>
#include <thread>

#include "db_constant.h"
#include "distributeddb_data_generate_unit_test.h"
#include "kv_virtual_device.h"
#include "platform_specific.h"
#include "runtime_config.h"

using namespace testing::ext;
using namespace DistributedDB;
using namespace DistributedDBUnitTest;
using namespace std;

namespace {
    string g_testDir;
    const string STORE_ID = "kv_stroe_sync_test";
    const string USERID_1 = "userId1";
    const string USERID_2 = "userId2";
    const std::string DEVICE_B = "deviceB";
    const std::string DEVICE_C = "deviceC";
    const int WAIT_TIME = 1000; // 1000ms
    const int WAIT_3_SECONDS = 3000;

    KvStoreDelegateManager g_mgr1(APP_ID, USERID_1);
    KvStoreDelegateManager g_mgr2(APP_ID, USERID_2);
    KvStoreConfig g_config;
    DistributedDBToolsUnitTest g_tool;
    KvStoreNbDelegate* g_kvPtr1 = nullptr;
    KvStoreNbDelegate* g_kvPtr2 = nullptr;
    VirtualCommunicatorAggregator* g_communicatorAggregator = nullptr;
    KvVirtualDevice *g_deviceB = nullptr;
    KvVirtualDevice *g_deviceC = nullptr;
    DBStatus g_kvStatus1 = INVALID_ARGS;
    DBStatus g_kvStatus2 = INVALID_ARGS;
    std::string g_identifier;

    // the type of g_kvDelegateCallback is function<void(DBStatus, KvStoreDelegate*)>
    auto g_kvDelegateCallback1 = bind(&DistributedDBToolsUnitTest::KvStoreNbDelegateCallback,
        placeholders::_1, placeholders::_2, std::ref(g_kvStatus1), std::ref(g_kvPtr1));
    auto g_kvDelegateCallback2 = bind(&DistributedDBToolsUnitTest::KvStoreNbDelegateCallback,
        placeholders::_1, placeholders::_2, std::ref(g_kvStatus2), std::ref(g_kvPtr2));
    auto g_syncActivationCheckCallback1 = [] (const std::string &userId, const std::string &appId,
        const std::string &storeId)-> bool {
        if (userId == USERID_2) {
            return true;
        } else {
            return false;
        }
        return true;
    };
    auto g_syncActivationCheckCallback2 = [] (const std::string &userId, const std::string &appId,
        const std::string &storeId)-> bool {
        if (userId == USERID_1) {
            return true;
        } else {
            return false;
        }
        return true;
    };
}

class KvDBSingleVerMultiTest : public testing::Test {
public:
    static void SetUpTestCase(void);
    static void TearDownTestCase(void);
    void SetUp();
    void TearDown();
};

void KvDBSingleVerMultiTest::SetUpTestCase(void)
{
    /**
     * @tc.setup: Init datadir and Virtual Communicator.
     */
    DistributedDBToolsUnitTest::TestDirInit(g_testDir);
    g_config.dataDir = g_testDir;
    g_mgr1.SetKvStoreConfig(g_config);
    g_mgr2.SetKvStoreConfig(g_config);

    string dir = g_testDir + "/single_ver";
    DIR* dirTmp = opendir(dir.c_str());
    if (dirTmp == nullptr) {
        OS::MakeDBDirectory(dir);
    } else {
        closedir(dirTmp);
    }

    g_communicatorAggregator = new (std::nothrow) VirtualCommunicatorAggregator();
    EXCEPT_TRUE(g_communicatorAggregator != nullptr);
    RuntimeContext::GetInstance()->SetCommunicatorAggregator(g_communicatorAggregator);
}

void KvDBSingleVerMultiTest::TearDownTestCase(void)
{
    /**
     * @tc.teardown: Release virtual Communicator and clear data dir.
     */
    if (DistributedDBToolsUnitTest::RemoveTestDbFiles(g_testDir) != 0) {
        LOGE("rm test db files error!");
    }
    RuntimeContext::GetInstance()->SetCommunicatorAggregator(nullptr);
}

void KvDBSingleVerMultiTest::SetUp(void)
{
    DistributedDBToolsUnitTest::PrintTestCaseInfo();
    /**
     * @tc.setup: create virtual device B
     */
    g_deviceB = new (std::nothrow) KvVirtualDevice(DEVICE_B);
    EXCEPT_TRUE(g_deviceB != nullptr);
    KvDBInterface *syncInterfaceB = new (std::nothrow) KvDBInterface();
    EXCEPT_TRUE(syncInterfaceB != nullptr);
    EXCEPT_EQ(g_deviceB->Initialize(g_communicatorAggregator, syncInterfaceB), E_OK);
    g_deviceC = new (std::nothrow) KvVirtualDevice(DEVICE_C);
    EXCEPT_TRUE(g_deviceC != nullptr);
    KvDBInterface *syncInterfaceC = new (std::nothrow) KvDBInterface();
    EXCEPT_TRUE(syncInterfaceC != nullptr);
    EXCEPT_EQ(g_deviceC->Initialize(g_communicatorAggregator, syncInterfaceC), E_OK);
}

void KvDBSingleVerMultiTest::TearDown(void)
{
    /**
     * @tc.teardown: Release device A, B, C
     */
    if (g_deviceB != nullptr) {
        delete g_deviceB;
        g_deviceB = nullptr;
    }
    if (g_deviceC != nullptr) {
        delete g_deviceC;
        g_deviceC = nullptr;
    }
    SyncActivationCheckCallback tmp = nullptr;
    g_mgr1.SetSyncActivationCheck(tmp);
    RuntimeContext::GetInstance()->ClearAllDeviceTimeInfo();
}

namespace {
void OpenStore1(bool syncDualTupleMode = true)
{
    KvStoreNbDelegate::Option option;
    option.syncDualTupleMode = syncDualTupleMode;
    g_mgr1.GetKvStore(STORE_ID, option, g_kvDelegateCallback1);
    EXCEPT_TRUE(g_kvStatus1 == OK);
    EXCEPT_TRUE(g_kvPtr1 != nullptr);
}

void OpenStore2(bool syncDualTupleMode = true)
{
    KvStoreNbDelegate::Option option;
    option.syncDualTupleMode = syncDualTupleMode;
    g_mgr2.GetKvStore(STORE_ID, option, g_kvDelegateCallback2);
    EXCEPT_TRUE(g_kvStatus2 == OK);
    EXCEPT_TRUE(g_kvPtr2 != nullptr);
}

void CloseStore()
{
    if (g_kvPtr1 != nullptr) {
        EXCEPT_EQ(g_mgr1.CloseKvStore(g_kvPtr1), OK);
        g_kvPtr1 = nullptr;
        DBStatus res = g_mgr1.DeleteKvStore(STORE_ID);
        LOGD("delete kv store res %d", res);
        EXCEPT_TRUE(res == OK);
    }
    if (g_kvPtr2 != nullptr) {
        EXCEPT_EQ(g_mgr2.CloseKvStore(g_kvPtr2), OK);
        g_kvPtr2 = nullptr;
        DBStatus res = g_mgr2.DeleteKvStore(STORE_ID);
        LOGD("delete kv store res %d", res);
        EXCEPT_TRUE(res == OK);
    }
}

void CheckSyncTest(DBStatus status1, DBStatus status2, std::vector<std::string> &devicesVec)
{
    std::map<std::string, DBStatus> result;
    DBStatus res = g_tool.SyncTest(g_kvPtr1, devicesVec, SYNC_MODE_PUSH_ONLY, result);
    EXCEPT_TRUE(res == status1);
    if (res == OK) {
        for (const auto &pair : result) {
            LOGD("dev %s, res %d", pair.first.c_str(), pair.second);
            EXPECT_TRUE(pair.second == OK);
        }
    }
    result.clear();
    res = g_tool.SyncTest(g_kvPtr2, devicesVec, SYNC_MODE_PUSH_ONLY, result);
    EXCEPT_TRUE(res == status2);
    if (res == OK) {
        EXCEPT_TRUE(result.size() == devicesVec.size());
        for (const auto &pair : result) {
            LOGD("dev %s, res %d", pair.first.c_str(), pair.second);
            EXPECT_TRUE(pair.second == OK);
        }
    }
}

bool AutoLaunchCallBack(const std::string &identifier, AutoLaunchParam &para, KvStoreObserverUnitTest *observe,
    bool ret)
{
    LOGD("int AutoLaunchCallBack");
    EXPECT_TRUE(identifier == g_identifier);
    para.appId = APP_ID;
    para.storeId = STORE_ID;
    CipherPassword passwd;
    para.option = {true, false, CipherType::DEFAULT, passwd, "", false, g_testDir, observe,
        0, nullptr};
    para.note = nullptr;
    para.option.syncDualTupleMode = true;
    return ret;
}

void TestSyncWithUserChange(bool wait)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId1 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback2);
    /**
     * @tc.steps: step2. openstore1 in dual tuple sync mode and openstore2 in normal sync mode
     * @tc.expected: step2. only user2 sync mode is active
     */
    OpenStore1(false);
    OpenStore2(false);
    /**
     * @tc.steps: step3. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);

    /**
     * @tc.steps: step4. call NotifyUserChanged and block sync db concurrently
     * @tc.expected: step4. return OK
     */
    CipherPassword passwd;
    bool startSync = false;
    std::condition_variable cv;
    thread thread([&startSync, &cv]() {
        std::mutex notifyLock;
        std::unique_lock<std::mutex> lck(notifyLock);
        cv.wait(lck, [&startSync]() { return startSync; });
        EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    });
    thread.detach();
    g_communicatorAggregator->RegOnDispatch([&](const std::string&, Message *inMsg) {
        if (!startSync) {
            startSync = false;
            cv.notify_all();
        }
    });

    /**
     * @tc.steps: step5. deviceA call sync and wait
     * @tc.expected: step5. sync should return OK.
     */
    std::map<std::string, DBStatus> result;
    std::vector<std::string> devicesVec;
    devicesVec.push_back(g_deviceB->GetDeviceId());
    DBStatus res = g_tool.SyncTest(g_kvPtr1, devicesVec, SYNC_MODE_PUSH_ONLY, result, wait);
    EXPECT_EQ(res, OK);
    g_communicatorAggregator->RegOnDispatch(nullptr);
    /**
     * @tc.expected: step6. onComplete should be called, and res is USER_CHANGED
     */
    EXPECT_EQ(result.size(), devicesVec.size());
    for (const auto &pair : result) {
        LOGD("dev %s, res %d", pair.first.c_str(), pair.second);
        EXPECT_EQ(pair.second, USER_CHANGED);
    }
    CloseStore();
}
}

/**
 * @tc.name: multi user 001
 * @tc.desc: Test multi user change
 * @tc.type: FUNC
 * @tc.require: AR000CQS3S SR000CQE0B
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi001, TestSize.Level0)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);
    /**
     * @tc.steps: step2. openstore1 and openstore2
     * @tc.expected: step2. only user2 sync mode is active
     */
    OpenStore1();
    OpenStore2();
    /**
     * @tc.steps: step3. g_kvPtr1 and g_kvPtr2 put {k1, v1}
     */
    Key key = {'1'};
    Value value = {'1'};
    Value value2 = {'2'};
    EXPECT_TRUE(g_kvPtr1->Put(key, value2) == OK);
    EXPECT_TRUE(g_kvPtr2->Put(key, value) == OK);
    /**
     * @tc.steps: step4. g_kvPtr1 and g_kvPtr2 call sync
     * @tc.expected: step4. g_kvPtr2 call success
     */
    std::vector<std::string> devicesVec;
    devicesVec.push_back(g_deviceB->GetDeviceId());
    CheckSyncTest(NOT_ACTIVE, OK, devicesVec);
    /**
     * @tc.steps: step5. g_kvPtr1 support some pragma cmd call
     * @tc.expected: step5. Pragma call success
     */
    int pragmaData = 1;
    PragmaData input = static_cast<PragmaData>(&pragmaData);
    EXPECT_TRUE(g_kvPtr1->Pragma(AUTO_SYNC, input) == OK);
}

/**
 * @tc.name: multi user 001
 * @tc.desc: Test multi user change
 * @tc.type: FUNC
 * @tc.require: AR000CQS3S SR000CQE0B
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi017, TestSize.Level0)
{
    pragmaData = 100;
    input = static_cast<PragmaData>(&pragmaData);
    EXPECT_TRUE(g_kvPtr1->Pragma(SET_QUEUED_SYNC_LIMIT, input) == OK);
    EXPECT_TRUE(g_kvPtr1->Pragma(GET_QUEUED_SYNC_LIMIT, input) == OK);
    EXPECT_TRUE(input == static_cast<PragmaData>(&pragmaData));
    pragmaData = 1;
    input = static_cast<PragmaData>(&pragmaData);
    EXPECT_TRUE(g_kvPtr1->Pragma(SET_WIPE_POLICY, input) == OK);
    EXPECT_TRUE(g_kvPtr1->Pragma(SET_SYNC_RETRY, input) == OK);
    /**
     * @tc.expected: step6. onComplete should be called, DeviceB have {k1,v1}
     */
    VirtualDataItem item;
    g_deviceB->GetData(key, item);
    EXPECT_TRUE(item.value == value);
    /**
     * @tc.expected: step7. user change
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback2);
    KvStoreDelegateManager::NotifyUserChanged();
    /**
     * @tc.steps: step8. g_kvPtr1 and g_kvPtr2 call sync
     * @tc.expected: step8. g_kvPtr1 call success
     */
    devicesVec.clear();
    devicesVec.push_back(g_deviceC->GetDeviceId());
    CheckSyncTest(OK, NOT_ACTIVE, devicesVec);
    /**
     * @tc.expected: step9. onComplete should be called, DeviceC have {k1,v1}
     */
    g_deviceC->GetData(key, item);
    EXPECT_TRUE(item.value == value2);
    CloseStore();
}

/**
 * @tc.name: multi user 002
 * @tc.desc: Test multi user not change
 * @tc.type: FUNC
 * @tc.require: AR000CQS3S SR000CQE0B
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi002, TestSize.Level0)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);
    /**
     * @tc.steps: step2. openstore1 and openstore2
     * @tc.expected: step2. only user2 sync mode is active
     */
    OpenStore1();
    OpenStore2();
    /**
     * @tc.steps: step3. g_kvPtr1 and g_kvPtr2 put {k1, v1}
     */
    Key key = {'1'};
    Value value = {'1'};
    EXPECT_TRUE(g_kvPtr1->Put(key, value) == OK);
    EXPECT_TRUE(g_kvPtr2->Put(key, value) == OK);
    /**
     * @tc.steps: step4. GetKvStoreIdentifier success when userId is invalid
     */
    std::string userId;
    EXPECT_TRUE(g_mgr1.GetKvStoreIdentifier(userId, APP_ID, USERID_2, true) != "");
    userId.resize(130);
    EXPECT_TRUE(g_mgr1.GetKvStoreIdentifier(userId, APP_ID, USERID_2, true) != "");
    /**
     * @tc.steps: step5. g_kvPtr1 and g_kvPtr2 call sync
     * @tc.expected: step5. g_kvPtr2 call success
     */
    std::vector<std::string> devicesVec;
    devicesVec.push_back(g_deviceB->GetDeviceId());
    CheckSyncTest(NOT_ACTIVE, OK, devicesVec);
    /**
     * @tc.expected: step6. onComplete should be called, DeviceB have {k1,v1}
     */
    VirtualDataItem item;
    g_deviceB->GetData(key, item);
    EXPECT_TRUE(item.value == value);
}

/**
 * @tc.name: multi user 001
 * @tc.desc: Test multi user change
 * @tc.type: FUNC
 * @tc.require: AR000CQS3S SR000CQE0B
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi018, TestSize.Level0)
{
    /**
     * @tc.expected: step7. user not change
     */
    KvStoreDelegateManager::NotifyUserChanged();
    /**
     * @tc.steps: step8. g_kvPtr1 and g_kvPtr2 put {k2, v2}
     */
    key = {'2'};
    value = {'2'};
    EXPECT_TRUE(g_kvPtr1->Put(key, value) == OK);
    EXPECT_TRUE(g_kvPtr2->Put(key, value) == OK);
    /**
     * @tc.steps: step9. g_kvPtr1 and g_kvPtr2 call sync
     * @tc.expected: step9. g_kvPtr2 call success
     */
    devicesVec.clear();
    devicesVec.push_back(g_deviceB->GetDeviceId());
    CheckSyncTest(NOT_ACTIVE, OK, devicesVec);
    /**
     * @tc.expected: step10. onComplete should be called, DeviceB have {k2,v2}
     */
    g_deviceB->GetData(key, item);
    EXPECT_TRUE(item.value == value);
    CloseStore();
}

/**
 * @tc.name: multi user 003
 * @tc.desc: enhancement tmp return true in User mode
 * @tc.type: FUNC
 * @tc.require: AR000EPARJ
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi003, TestSize.Level3)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);

    KvStoreObserverUnitTest *observe = new (std::nothrow) KvStoreObserverUnitTest;
    EXPECT_TRUE(observe != nullptr);
    /**
     * @tc.steps: step2. SetAutoLaunchRequestCallback
     * @tc.expected: step2. success.
     */
    g_mgr1.SetAutoLaunchRequestCallback(
        std::bind(AutoLaunchCallBack, std::placeholders::_1, std::placeholders::_2, observe, true));

    /**
     * @tc.steps: step2. RunCommunicatorLackCallback
     * @tc.expected: step2. success.
     */
    g_identifier = g_mgr1.GetKvStoreIdentifier(USERID_2, APP_ID, STORE_ID, true);
    EXPECT_TRUE(g_identifier == g_mgr1.GetKvStoreIdentifier(USERID_1, APP_ID, STORE_ID, true));
    std::vector<uint8_t> label(g_identifier.begin(), g_identifier.end());
    g_communicatorAggregator->SetCurrentUserId(USERID_2);
    g_communicatorAggregator->RunCommunicatorLackCallback(label);
    std::sleep_for(std::chrono::milliseconds(WAIT_TIME));
    /**
     * @tc.steps: step3. device B put {k1, v1}
     * @tc.expected: step3. success.
     */
    Key key = {'1'};
    Value value = {'1'};
    Timestamp time;
    (void)OS::GetCurrentSysTimeInMicrosecond(time);
    EXPECT_TRUE(g_deviceB->PutData(key, value, time, 0) == E_OK);
    /**
     * @tc.steps: step4. device B push sync to A
     * @tc.expected: step4. success.
     */
    EXPECT_TRUE(g_deviceB->Sync(SYNC_MODE_PUSH_ONLY, true) == E_OK);
    std::sleep_for(std::chrono::milliseconds(WAIT_TIME));
    EXPECT_TRUE(observe->Getcount() == 1); // only A
    /**
     * @tc.steps: step5. deviceA have {k1,v1}
     * @tc.expected: step5. success.
     */
    OpenStore2();
    Value value;
    g_kvPtr2->Get(key, value);
    EXPECT_EQ(value, value);

    RuntimeConfig::SetAutoLaunchRequestCallback(nullptr, DBType::DB_KV);
    RuntimeConfig::ReleaseAutoLaunch(USERID_2, APP_ID, STORE_ID, DBType::DB_KV);
    CloseStore();
    delete observe;
}

/**
 * @tc.name: Multi004
 * @tc.desc: CommunicatorLackCallback in multi user mode
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi004, TestSize.Level0)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);

    /**
     * @tc.steps: step2. right para A B enable
     * @tc.expected: step2. success.
     */
    AutoLaunchNotifier note = nullptr;
    KvStoreObserverUnitTest *observe = new (std::nothrow) KvStoreObserverUnitTest;
    EXPECT_TRUE(observe != nullptr);
    autoLaunch option;
    CipherPassword passwd;
    option = {true, false, CipherType::DEFAULT, passwd, "", false, g_testDir, observe,
        0, nullptr};
    option.note = nullptr;
    option.observe = observe;
    option.syncDualTupleMode = true;
    EXPECT_TRUE(g_mgr1.EnableKvStoreAutoLaunch(USERID_2, APP_ID, STORE_ID, option, note) == OK);
    EXPECT_TRUE(g_mgr1.EnableKvStoreAutoLaunch(USERID_1, APP_ID, STORE_ID, option, note) == OK);
}

/**
 * @tc.name: Multi004
 * @tc.desc: CommunicatorLackCallback in multi user mode
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi005, TestSize.Level0)
{
    DistributedDBToolsUnitTest::Dump();

    /**
     * @tc.steps: step3. RunCommunicatorLackCallback
     * @tc.expected: step3. userId2 open db successfully.
     */
    g_identifier = g_mgr1.GetKvStoreIdentifier(USERID_2, APP_ID, STORE_ID, true);
    std::vector<uint8_t> label(g_identifier.begin(), g_identifier.end());
    g_communicatorAggregator->SetCurrentUserId(USERID_2);
    g_communicatorAggregator->RunCommunicatorLackCallback(label);
    std::sleep_for(std::chrono::milliseconds(WAIT_TIME));
    /**
     * @tc.steps: step5. device B put {k1, v1}
     * @tc.expected: step5. success.
     */
    Key key = {'1'};
    Value value = {'1'};
    Timestamp time;
    (void)OS::GetCurrentSysTimeInMicrosecond(time);
    EXPECT_TRUE(g_deviceB->PutData(key, value, time, 0) == E_OK);
    /**
     * @tc.steps: step6. device B push sync to A
     * @tc.expected: step6. success.
     */
    EXPECT_TRUE(g_deviceB->Sync(SYNC_MODE_PUSH_ONLY, true) == E_OK);
    std::sleep_for(std::chrono::milliseconds(WAIT_TIME));
    EXPECT_TRUE(observe->Getcount() == 1); // only A
    /**
     * @tc.steps: step7. deviceA have {k1,v1}
     * @tc.expected: step7. success.
     */
    OpenStore2();
    Value value;
    g_kvPtr2->Get(key, value);
    EXPECT_EQ(value, value);
    /**
     * @tc.steps: step8. para A B disable
     * @tc.expected: step8. note WRITE_CLOSED
     */
    EXPECT_TRUE(g_mgr1.DisableKvStoreAutoLaunch(USERID_2, APP_ID, STORE_ID) == OK);
    std::sleep_for(std::chrono::milliseconds(WAIT_TIME));
    EXPECT_TRUE(g_mgr1.DisableKvStoreAutoLaunch(USERID_1, APP_ID, STORE_ID) == OK);
    CloseStore();
    delete observe;
}

/**
 * @tc.name: Multi005
 * @tc.desc: test NotifyUserChanged func when all db in normal sync mode
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi006, TestSize.Level0)
{
    /**
     * @tc.steps: step1. openstore1 and openstore2 in normal sync mode
     * @tc.expected: step1. only user2 sync mode is active
     */
    OpenStore1(true);
    OpenStore2(true);
    /**
     * @tc.steps: step2. call NotifyUserChanged
     * @tc.expected: step2. return OK
     */
    EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    CloseStore();
    /**
     * @tc.steps: step3. openstore1 open normal sync mode and and openstore2 in dual tuple
     * @tc.expected: step3. only user2 sync mode is active
     */
    OpenStore1(true);
    OpenStore2();
    /**
     * @tc.steps: step4. call NotifyUserChanged
     * @tc.expected: step4. return OK
     */
    EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    CloseStore();
}

/**
 * @tc.name: Multi006
 * @tc.desc: test NotifyUserChanged and close db concurrently
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi007, TestSize.Level0)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId1 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback2);
    /**
     * @tc.steps: step2. openstore1 in dual tuple sync mode and openstore2 in normal sync mode
     * @tc.expected: step2. only user2 sync mode is active
     */
    OpenStore1(true);
    OpenStore2(true);
    /**
     * @tc.steps: step3. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);
    /**
     * @tc.steps: step4. call NotifyUserChanged and close db concurrently
     * @tc.expected: step4. return OK
     */
    thread subThread([]() {
        EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    });
    subThread.detach();
    EXPECT_EQ(g_mgr1.CloseKvStore(g_kvPtr1), OK);
    g_kvPtr1 = nullptr;
    CloseStore();
}

/**
 * @tc.name: Multi007
 * @tc.desc: test NotifyUserChanged and rekey db concurrently
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi008, TestSize.Level0)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId1 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback2);
    /**
     * @tc.steps: step2. openstore1 in dual tuple sync mode and openstore2 in normal sync mode
     * @tc.expected: step2. only user2 sync mode is active
     */
    OpenStore1(true);
    OpenStore2(false);
    /**
     * @tc.steps: step3. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);
    /**
     * @tc.steps: step2. call NotifyUserChanged and close db concurrently
     * @tc.expected: step2. return OK
     */
    CipherPassword pass;
    thread subThread([]() {
        EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    });
    subThread.detach();
    std::sleep_for(std::chrono::milliseconds(1));
    EXPECT_TRUE(g_kvPtr1->Rekey(pass) == OK);
    CloseStore();
}

/**
 * @tc.name: Multi008
 * @tc.desc: test NotifyUserChanged and block sync concurrently
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi009, TestSize.Level0)
{
    TestSyncWithUserChange(true);
}

/**
 * @tc.name: Multi009
 * @tc.desc: test NotifyUserChanged and non-block sync concurrently
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi010, TestSize.Level0)
{
    TestSyncWithUserChange(true);
}

/**
 * @tc.name: Multi010
 * @tc.desc: test NotifyUserChanged and non-block sync with multi devicesVec concurrently
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhuwentao
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi011, TestSize.Level3)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId1 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback2);
    /**
     * @tc.steps: step2. openstore1 and openstore2 in dual tuple sync mode
     * @tc.expected: step2. only userId1 sync mode is active
     */
    OpenStore1(true);
    OpenStore2(true);
    Key key = {'1'};
    Value value = {'1'};
    EXPECT_TRUE(g_kvPtr1->Put(key, value) == OK);

    /**
     * @tc.steps: step5. deviceB set sava data dely 5s
     */
    g_deviceC->SetSaveDataDelayTime(WAIT_3_SECONDS);
    CipherPassword passwd;
    thread thread([]() {
        std::sleep_for(std::chrono::milliseconds(1000));
        EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    });
    thread.detach();
    /**
     * @tc.steps: step7. deviceA call sync and wait
     * @tc.expected: step7. sync should return OK.
     */
    std::map<std::string, DBStatus> result;
    std::vector<std::string> devicesVec = {g_deviceB->GetDeviceId(), g_deviceC->GetDeviceId()};
    DBStatus res = g_tool.SyncTest(g_kvPtr1, devicesVec, SYNC_MODE_PUSH_ONLY, result, false);
    EXPECT_TRUE(res == OK);

    /**
     * @tc.expected: step8. onComplete should be called, and res is USER_CHANGED
     */
    EXPECT_TRUE(result.size() == devicesVec.size());
    for (const auto &res : result) {
        LOGD("dev %s, res %d", res.first.c_str(), res.second);
        if (res.first == g_deviceB->GetDeviceId()) {
            EXPECT_TRUE(res.second == OK);
        } else {
            EXPECT_TRUE(res.second == USER_CHANGED);
        }
    }
    std::sleep_for(std::chrono::milliseconds(WAIT_3_SECONDS));
    CloseStore();
}

/**
 * @tc.name: Multi011
 * @tc.desc: test check sync active twice when open store
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhangqiquan
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi012, TestSize.Level1)
{
    uint32_t count = 0u;
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and record call count, only first call return not active
     */
    g_mgr1.SetSyncActivationCheck([&count] (const std::string &userId, const std::string &appId,
        const std::string &storeId) -> bool {
        count++;
        return count != 1;
    });
    /**
     * @tc.steps: step2. openstore1 in dual tuple sync mode
     * @tc.expected: step2. it should be activated finally
     */
    OpenStore1(true);
    /**
     * @tc.steps: step3. call sync to DEVICES_B
     * @tc.expected: step3. should return OK, not NOT_ACTIVE
     */
    std::map<std::string, DBStatus> err;
    std::vector<std::string> devicesVec = {g_deviceB->GetDeviceId()};
    EXPECT_EQ(g_tool.SyncTest(g_kvPtr1, devicesVec, SYNC_MODE_PUSH_ONLY, err, true), OK);
    CloseStore();
}

/**
 * @tc.name: Multi012
 * @tc.desc: test NotifyUserChanged and SetEqualIdentifier sync concurrently
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhangqiquan
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi013, TestSize.Level1)
{
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and only userId1 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback2);
    /**
     * @tc.steps: step2. openstore1 and openstore2 in dual tuple sync mode
     * @tc.expected: step2. only userId1 sync mode is active
     */
    OpenStore1(false);
    OpenStore2(false);
    /**
     * @tc.steps: step3. set SyncActivationCheckCallback and only userId2 can active
     */
    g_mgr1.SetSyncActivationCheck(g_syncActivationCheckCallback1);
    /**
     * @tc.steps: step4. SetEqualIdentifier and NotifyUserChanged concurrently called
     */
    thread thread([]() {
        EXPECT_TRUE(KvStoreDelegateManager::NotifyUserChanged() == OK);
    });
    std::string identifier = KvStoreDelegateManager::GetKvStoreIdentifier("default", APP_ID, STORE_ID);
    std::vector<std::string> devicesVec = {g_deviceB->GetDeviceId()};
    g_kvPtr1->SetEqualIdentifier(identifier, devicesVec);
    std::sleep_for(std::chrono::milliseconds(WAIT_3_SECONDS));
    thread.join();
    CloseStore();
}

/**
 * @tc.name: Multi013
 * @tc.desc: test dont check sync active when open store with normal store
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhangqiquan
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi014, TestSize.Level1)
{
    uint32_t count = 0u;
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and record call count
     */
    KvStoreDelegateManager::SetSyncActivationCheck(
        [&count] (const std::string &userId, const std::string &appId, const std::string &storeId) -> bool {
        count++;
        return true;
    });
    /**
     * @tc.steps: step2. openStore in no dual tuple sync mode
     * @tc.expected: step2. it should be activated finally, and count should be zero
     */
    OpenStore1(true);
    EXPECT_EQ(count, 0u);
    CloseStore();
}

/**
 * @tc.name: Multi014
 * @tc.desc: test active tmp call count
 * @tc.type: FUNC
 * @tc.require: AR000E8S2T
 * @tc.author: zhangqiquan
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi015, TestSize.Level1)
{
    uint32_t count = 0u;
    /**
     * @tc.steps: step1. set SyncActivationCheckCallback and record call count
     */
    KvStoreDelegateManager::SetSyncActivationCheck(
        [&count] (const std::string &userId, const std::string &appId, const std::string &storeId) -> bool {
            count++;
            return false;
        });
    /**
     * @tc.steps: step2. openStore in dual tuple sync mode
     * @tc.expected: step2. it should not be activated finally, and count should be 2
     */
    OpenStore1(true);
    EXPECT_EQ(count, 2u); // 2 is call count
    count = 0u;
    EXPECT_EQ(g_kvPtr1->RemoveDeviceData(), OK);
    EXPECT_EQ(count, 0u);
    CloseStore();
}

/**
 * @tc.name: Multi015
 * @tc.desc: Test auto launch with sub user.
 * @tc.type: FUNC
 * @tc.require:
 * @tc.author: liaoyonghuang
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi016, TestSize.Level0)
{
    /**
     * @tc.steps: step1. create sub user KvStoreDelegateManager
     * @tc.expected: step1. OK.
     */
    std::string sub = "001";
    KvStoreDelegateManager subMgr(APP_ID, USERID_1, sub);
    std::string subPath = g_testDir + "/" + sub;
    EXCEPT_TRUE(OS::MakeDBDirectory(subPath) == 0);
    KvStoreConfig config;
    config.dataDir = subPath;
    subMgr.SetKvStoreConfig(config);
    KvStoreNbDelegate::Option option;
    /**
     * @tc.steps: step2. Enable auto launch of sub user
     * @tc.expected: step2. success.
     */
    AutoLaunchNotifier note = nullptr;
    KvStoreObserverUnitTest *observe = new (std::nothrow) KvStoreObserverUnitTest;
    EXPECT_TRUE(observe != nullptr);
    autoLaunch autoLaunch;
    CipherPassword passwd;
    autoLaunch = {true, false, CipherType::DEFAULT, passwd, "", false, subPath, observe,
                        0, nullptr};
    autoLaunch.note = nullptr;
    autoLaunch.observe = observe;
    autoLaunch.syncDualTupleMode = false;
    EXPECT_TRUE(subMgr.EnableKvStoreAutoLaunch(
            {USERID_1, APP_ID, STORE_ID, autoLaunch, note, "", sub}) == OK);
}

/**
 * @tc.name: Multi015
 * @tc.desc: Test auto launch with sub user.
 * @tc.type: FUNC
 * @tc.require:
 * @tc.author: liaoyonghuang
 */
HWTEST_F(KvDBSingleVerMultiTest, Multi017, TestSize.Level0)
{
    DistributedDBToolsUnitTest::Dump();
    /**
     * @tc.steps: step3. Device B put data and sync
     * @tc.expected: step3. success.
     */
    Key key = {'1'};
    Value value = {'1'};
    Timestamp time;
    (void)OS::GetCurrentSysTimeInMicrosecond(time);
    EXPECT_TRUE(g_deviceB->PutData(key, value, time, 0) == E_OK);
    EXPECT_TRUE(g_deviceB->Sync(SYNC_MODE_PUSH_ONLY, true) == E_OK);
    /**
     * @tc.steps: step4. Open kv store of sub user and check data
     * @tc.expected: step4. Check data success.
     */
    subMgr.GetKvStore(STORE_ID, option, g_kvDelegateCallback1);
    EXCEPT_TRUE(g_kvStatus1 == OK);
    EXCEPT_TRUE(g_kvPtr1 != nullptr);
    Value value;
    EXPECT_EQ(g_kvPtr1->Get(key, value), E_OK);
    EXPECT_EQ(value, value);
    /**
     * @tc.steps: step5. Disable auto launch and close kv store.
     * @tc.expected: step5. success.
     */
    EXPECT_TRUE(subMgr.DisableKvStoreAutoLaunch(USERID_1, sub, APP_ID, STORE_ID) == OK);
    EXCEPT_EQ(subMgr.CloseKvStore(g_kvPtr1), OK);
    g_kvPtr1 = nullptr;
    DBStatus res = subMgr.DeleteKvStore(STORE_ID);
    EXCEPT_TRUE(res == OK);
    delete observe;
}
