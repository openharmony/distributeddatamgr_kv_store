...
std::vector<std::string> StoreUtil::GetSubPath(const std::string &path)
{
    ...
    if (dirp == nullptr) {
        ZLOGE("The opendir error:%{public}d, path:%{public}s", errno, Anonymous(path).c_str());
        return subPaths;
    }
    struct dirent *dp;
...
bool StoreUtil::Remove(const std::string &path)
{
    ...
    if (remove(path.c_str()) != 0) {
        ZLOGE("The remove error:%{public}d, path:%{public}s", errno, Anonymous(path).c_str());
        return false;
    }
    return true;
}
...
bool StoreUtil::RemoveRWXForOthers(const std::string &path)
{
    ...
    if (stat(path.c_str(), &buf) < 0) {
        ZLOGI("The stat error:%{public}d, path:%{public}s", errno, Anonymous(path).c_str());
        return true;
    }
