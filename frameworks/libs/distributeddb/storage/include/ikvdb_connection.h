...
public:
    ...
    virtual int RegisterObserverAction(const std::weak_ptr<KvStoreObserver> &observer,
        const ObserverAction &action) = 0;

    virtual int UnRegisterObserverAction(const std::weak_ptr<KvStoreObserver> &observer) = 0;
