...
int GenericKvDBConnection::GetSyncDataSize(const std::string &device, size_t &size) const
{
    return -E_NOT_SUPPORT;
}
...
int GenericKvDBConnection::RegisterObserverAction([[gnu::unused]] const std::shared_ptr<KvStoreObserver> &observer,
    [[gnu::unused]] const ObserverAction &action)
{
    return E_OK;
}

int GenericKvDBConnection::UnRegisterObserverAction([[gnu::unused]] const std::shared_ptr<KvStoreObserver> &observer)
{
    return E_OK;
}
