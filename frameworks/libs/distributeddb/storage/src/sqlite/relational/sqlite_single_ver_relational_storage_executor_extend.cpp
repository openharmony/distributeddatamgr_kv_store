    ...
    const TableSchema &tableSchema)
{
    ...
    std::vector<std::string> localPkNames =
        SQLiteRelationalUtils::GetLocalPkNames(localSchema_.GetTable(tableSchema.name));
    if (!localPkNames.empty()) {
        sql += " returning ";
        for (const auto &pk : localPkNames) {
            sql += pk + ",";
        }
        sql.pop_back();
    }
    ...
    const TrackerTable &trackerTable, int64_t dataKey)
{
    ...
    errCode = SQLiteRelationalUtils::SqliteStepReturningValues(SQLiteRelationalUtils::GetLocalPkNames(
        localSchema_.GetTable(tableSchema.name)), insertStmt, vBucket);
    SQLiteUtils::ResetStatement(insertStmt, true, ret);
    if (errCode != SQLiteUtils::MapSQLiteErrno(SQLITE_DONE)) {
    ...
    VBucket &vBucket)
{
    ...
    // update data
    errCode = SQLiteRelationalUtils::SqliteStepReturningValues(SQLiteRelationalUtils::GetLocalPkNames(
        localSchema_.GetTable(tableSchema.name)), updateStmt, vBucket);
    int ret = E_OK;
    SQLiteUtils::ResetStatement(updateStmt, true, ret);
    if (errCode != SQLiteUtils::MapSQLiteErrno(SQLITE_DONE)) {
...
int SQLiteSingleVerRelationalStorageExecutor::DeleteCloudData(const std::string &tableName, VBucket &vBucket,
