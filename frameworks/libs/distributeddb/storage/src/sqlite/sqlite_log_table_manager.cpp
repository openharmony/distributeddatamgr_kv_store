...
void SqliteLogTableManager::GetIndexSql(const TableInfo &table, std::vector<std::string> &schema)
{
    ...
    std::string indexTimestampFlag = "CREATE INDEX IF NOT EXISTS " +
        tableName + "_time_flag_index ON " + tableName + "(timestamp, flag);";
    schema.emplace_back(indexTimestampFlag);

    std::string indexHashkey = "CREATE INDEX IF NOT EXISTS " +
        tableName + "_hashkey_index ON " + tableName + "(hash_key);";
