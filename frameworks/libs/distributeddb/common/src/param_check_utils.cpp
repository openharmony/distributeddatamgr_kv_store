    ...
    const std::string &subUser, bool allowStoreIdWithDot)
{
    ...
    // subUser allow empty
    if (subUser.length() > DBConstant::MAX_SUB_USER_LENGTH) {
        LOGE("Invalid subUser info[%zu][%zu]", userId.length(), subUser.length());
        return false;
    }

    if ((appId.find(DBConstant::ID_CONNECTOR) != std::string::npos) ||
        (storeId.find(DBConstant::ID_CONNECTOR) != std::string::npos) ||
        (subUser.find(DBConstant::ID_CONNECTOR) != std::string::npos)) {
...
uint8_t ParamCheckUtils::GetValidCompressionRate(uint8_t compressionRate)
{
    ...
    if (compressionRate < 1 || compressionRate > DBConstant::DEFAULT_COMPTRESS_RATE) {
        LOGW("Invalid compression rate:%" PRIu8, compressionRate);
