...
/**
 * @tc.name: QuerySyncRetry001
 * @tc.desc: use sync retry sync use query pull by compress
 * @tc.type: FUNC
 * @tc.require:
 * @tc.author: zhangqiquan
 */
HWTEST_F(DistributedDBSingleVerP2PSimpleSyncTest, QuerySyncRetry001, TestSize.Level3)
{
    if (g_kvDelegatePtr != nullptr) {
        ASSERT_EQ(g_mgr.CloseKvStore(g_kvDelegatePtr), OK);
        g_kvDelegatePtr = nullptr;
    ...
    g_communicatorAggregator->SetDropMessageTypeByDevice(DEVICE_B, QUERY_SYNC_MESSAGE);
    std::vector<std::string> devices;
    devices.push_back(g_deviceB->GetDeviceId());
     ...
     */
    std::map<std::string, DBStatus> result;
    auto query = Query::Select().PrefixKey({});
    ASSERT_TRUE(g_tool.SyncTest(g_kvDelegatePtr, devices, SYNC_MODE_PULL_ONLY, result, query) == OK);
