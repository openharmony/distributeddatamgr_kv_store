...
HWTEST_F(DistributedDBStorageRdSingleVerNaturalExecutorTest, PragmaGetPageSize_Normal, TestSize.Level1)
{
    ...
    // Common pageSize enum values: 4096 / 8192 / 16384 / 65536 (depending on kernel compile config)
...
/**
  * @tc.name: PragmaGetPageSize_ImplicitCompat
  * @tc.desc: Verify kernel implicit compatibility for wrong pageSize open: when db exists, reopen with
  *           different pageSize does not report error, still opens with old pageSize
  * @tc.type: FUNC
  * @tc.require:
  */
HWTEST_F(DistributedDBStorageRdSingleVerNaturalExecutorTest, PragmaGetPageSize_ImplicitCompat, TestSize.Level1)
{
    // 1. Get actual pageSize of current database
    int originalPageSize = 0;
    int ret = g_connection->Pragma(PRAGMA_GET_PAGE_SIZE, &originalPageSize);
    EXPECT_EQ(ret, E_OK);
    EXPECT_GT(originalPageSize, 0);

    // 2. Release current test infrastructure, keep database files (do not delete)
    if (g_nullHandle != nullptr) {
        delete g_nullHandle;
        g_nullHandle = nullptr;
    }
    ...
    g_store = nullptr; // Avoid TearDown accessing possibly released store

    // 3. Reopen database with different pageSize property
    KvDBProperties newProp = g_property;
    int differentPageSize = (originalPageSize == 4096) ? 8192 : 4096;
    newProp.SetIntProp(KvDBProperties::KVDB_PAGE_SIZE, differentPageSize);

    RdSingleVerNaturalStore *newStore = new (std::nothrow) RdSingleVerNaturalStore;
    ASSERT_NE(newStore, nullptr);
    ret = newStore->Open(newProp);
    EXPECT_EQ(ret, E_OK); // Key assertion: implicit compatibility, no error

    int errCode = E_OK;
    auto *newConn = static_cast<RdSingleVerNaturalStoreConnection *>(newStore->GetDBConnection(errCode));
    ASSERT_NE(newConn, nullptr);
    RefObject::DecObjRef(newStore);

    // 4. Verify actual pageSize is still the old value, not the newly set value
    int actualPageSize = 0;
    ret = newConn->Pragma(PRAGMA_GET_PAGE_SIZE, &actualPageSize);
    EXPECT_EQ(ret, E_OK);
    EXPECT_EQ(actualPageSize, originalPageSize);

    // 5. Clean up new store to avoid file occupancy affecting TearDownTestCase
    newConn->Close();
}
