...
void UpdateDocOneFuzz(std::string s, const std::string input)
{
    ...
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, input.c_str(), "{}", 0);
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, "{}", input.c_str(), 0);
    GRD_UpdateDocInner(nullptr, COLLECTION_NAME, "{}", "{}", 0);
...
void UpdateDocTwoFuzz(const char *newCollName, std::string s, const std::string input)
{
    ...
    GRD_InsertDocInner(g_db, COLLECTION_NAME, inputJson.c_str(), 0);
    GRD_UpdateDocInner(g_db, inputJson.c_str(), "{}", "{}", 0);
    GRD_UpdateDocInner(g_db, newCollName, input.c_str(), "{}", 0);
    GRD_UpdateDocInner(g_db, newCollName, "{}", input.c_str(), 0);
    GRD_UpdateDocInner(nullptr, newCollName, "{}", "{}", 0);
    inputJson = "{\"_id\":\"2\", \"field\":" + input + "}";
    GRD_UpdateDocInner(g_db, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), 0);
    inputJson = "{\"field\":" + input + "}";
    GRD_UpdateDocInner(g_db, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), 0);
    ...
    GRD_UpdateDocInner(g_db, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), 0);
    inputJson = "{\"name\":\"doc6\", \"c0\" : {\"c1\" : true } }";
    GRD_InsertDocInner(g_db, newCollName, inputJson.c_str(), 0);
...
void UpdateDocFilterFuzz()
{
    ...
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter, updata2, 0);

    const char *filter1 = "{\"_id\" : \"1\"}";
    const char *updata1 = "{\"_id\" : \"6\"}";
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter1, updata1, 0);
    ...
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter2, updata3, 0);
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter2, updata4, 0);
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter2, updata5, 0);
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter2, updata6, 0);

    const char *filter3 = "{\"_id\" : \"1\"}";
    const char *updata7 = "{\"age\" : 21}";
    GRD_UpdateDocInner(g_db, NULL, filter3, updata7, 0);
    GRD_UpdateDocInner(g_db, "", filter3, updata7, 0);
    GRD_UpdateDocInner(NULL, COLLECTION_NAME, filter3, updata7, 0);
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, NULL, updata7, 0);
    GRD_UpdateDocInner(g_db, COLLECTION_NAME, filter3, NULL, 0);
    GRD_DropCollectionInner(g_db, COLLECTION_NAME, 0);
}
...
void UpsertDocNewFuzz(const std::string input, GRD_DB *db1)
{
    ...
    GRD_UpsertDocInner(g_db, "student", R""({"_id":"10001"})"", documentNew.c_str(), 0);
    GRD_UpsertDocInner(db1, "student", R""({"_id":"10001"})"", updateDocNew.c_str(), 1);
    GRD_DropCollectionInner(g_db, "student", 0);

    GRD_CreateCollectionInner(g_db, "student", "", 0);
    documentNew = "{\"name\":[\"Tmn\", \"BB\", \"Alice\"], \"field\":" + input + "}";
    updateDocNew = R""({"name.first":"GG"})"";
    GRD_UpsertDocInner(g_db, "student", R""({"_id":"10002"})"", documentNew.c_str(), 0);
...
void UpsertDocFuzz(const uint8_t *data, size_t size)
{
    ...
    GRD_InsertDocInner(g_db, COLLECTION_NAME, inputJson.c_str(), 0);
    GRD_UpsertDocInner(g_db, input.c_str(), "{}", "{}", 0);
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, inputJson.c_str(), "{}", 0);
    inputJson = "{\"_id\":\"1\", \"field\":" + input + "}";
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"1\"}", inputJson.c_str(), 0);
    inputJson = "{\"_id\":\"1\", \"field\":\"" + input + "\"}";
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"1\"}", inputJson.c_str(), 0);
    inputJson = "{\"field\":" + input + "}";
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"1\"}", inputJson.c_str(), 0);
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"2\"}", inputJson.c_str(), 0);
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"1\"}", inputJson.c_str(), 1);
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"2\"}", inputJson.c_str(), 1);
    GRD_UpsertDocInner(nullptr, COLLECTION_NAME, "{\"_id\":\"2\"}", inputJson.c_str(), 1);
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, nullptr, inputJson.c_str(), 1);
    GRD_UpsertDocInner(g_db, COLLECTION_NAME, "{\"_id\":\"2\"}", inputJson.c_str(), BATCH_COUNT);

    const char *newCollName = "./student";
    GRD_DB *db1 = nullptr;
    GRD_CreateCollectionInner(db1, newCollName, "", 0);
    GRD_InsertDocInner(db1, newCollName, inputJson.c_str(), 0);
    GRD_UpsertDocInner(db1, input.c_str(), "{}", "{}", 0);
    GRD_UpsertDocInner(db1, newCollName, inputJson.c_str(), "{}", 0);
    inputJson = "{\"_id\":\"1\", \"field\":" + input + "}";
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"1\"}", inputJson.c_str(), 0);
    inputJson = "{\"_id\":\"1\", \"field\":\"" + input + "\"}";
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"1\"}", inputJson.c_str(), 0);
    inputJson = "{\"field\":" + input + "}";
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"1\"}", inputJson.c_str(), 0);
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), 0);
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"1\"}", inputJson.c_str(), 1);
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), 1);
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"newID1999\"}", inputJsonNoId.c_str(), 0);
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"newID1999a\"}", inputJsonNoId.c_str(), 1);
    GRD_UpsertDocInner(nullptr, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), 1);
    GRD_UpsertDocInner(db1, newCollName, nullptr, inputJson.c_str(), 1);
    GRD_UpsertDocInner(db1, newCollName, "{\"_id\":\"2\"}", inputJson.c_str(), BATCH_COUNT);
    GRD_DropCollectionInner(db1, newCollName, 0);
    GRD_DropCollectionInner(g_db, COLLECTION_NAME, 0);
...
void DeleteDocResultFuzz(const std::string input)
{
    ...
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, filter1, 0);
    std::string filter2 = "{\"_id\" : \"1\", \"name\" : " + input + "}";
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, filter2.c_str(), 1);
    GRD_DeleteDocInner(g_db, NULL, filter2.c_str(), 0);
    GRD_DeleteDocInner(g_db, "", filter2.c_str(), 1);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, filter2.c_str(), 0);

    const char *filter3 = "{\"_id\" : \"1\"}";
    GRD_ResultSet *resultSet = nullptr;
    Query query = { filter3, "{}" };
    GRD_FindDocInner(g_db, COLLECTION_NAME, query, 0, &resultSet);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, filter3, 0);

    const char *filter4 = "{\"_id\" : \"1\"}";
    std::string collectionName1(LESS_HALF_BYTES, 'a');
    GRD_CreateCollectionInner(g_db, collectionName1.c_str(), "", 0);
    GRD_DeleteDocInner(g_db, collectionName1.c_str(), filter4, 0);
    GRD_DropCollectionInner(g_db, collectionName1.c_str(), 0);
    GRD_FreeResultSetInner(resultSet);

    GRD_DeleteDocInner(NULL, COLLECTION_NAME, filter2.c_str(), 0);
    GRD_DeleteDocInner(g_db, NULL, filter2.c_str(), 0);
    GRD_DeleteDocInner(g_db, "", filter2.c_str(), 0);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, NULL, 0);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, "", 0);
    GRD_DeleteDocInner(g_db, "notExisted", filter2.c_str(), 0);
    ...
    for (const auto &item : filterVec) {
        GRD_DeleteDocInner(g_db, COLLECTION_NAME, item.c_str(), 0);
    }
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, filter2.c_str(), 0);
    ...
    query = { filter2.c_str(), "{}" };
    GRD_FindDocInner(g_db, COLLECTION_NAME, query, 0, &resultSet);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, filter5.c_str(), 0);
    GRD_DropCollectionInner(g_db, COLLECTION_NAME, 0);
    GRD_FreeResultSetInner(resultSet);
}
...
void DeleteDocFuzz(const uint8_t *data, size_t size)
{
    ...
    GRD_DeleteDocInner(g_db, nullptr, "{}", 0);
    std::string longName = "";
    for (int i = 0; i < MORE_HALF_BYTES; i++) {
        longName += 'a';
    }
    GRD_DeleteDocInner(g_db, "grd_name", "{}", 0);
    GRD_DeleteDocInner(g_db, longName.c_str(), "{}", 0);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, inputJson.c_str(), 0);
    ...
    GRD_InsertDocInner(g_db, COLLECTION_NAME, document1, 0);
    GRD_InsertDocInner(g_db, COLLECTION_NAME, document2, 0);
    GRD_InsertDocInner(g_db, COLLECTION_NAME, document3, 0);
    GRD_DeleteDocInner(g_db, COLLECTION_NAME, "{}", 0);
...
void DbOpenOneFuzz(GRD_DB *dbVal)
{
    ...
    GRD_DBOpenInner(path.c_str(), "", GRD_DB_OPEN_ONLY, &dbVal);
    (void)remove(path.c_str());

    path = "./document.db";
    GRD_DBOpenInner(path.c_str(), R""({"pageSize":64, "redopubbufsize":4033})"", GRD_DB_OPEN_CREATE, &dbVal);
    GRD_DBCloseInner(dbVal, 0);
    GRD_DBOpenInner(path.c_str(), R""({"pageSize":64, "redopubbufsize":4032})"", GRD_DB_OPEN_CREATE, &dbVal);
    GRD_DBOpenInner(path.c_str(), R""({"redopubbufsize":255})"", GRD_DB_OPEN_CREATE, &dbVal);
    GRD_DBOpenInner(path.c_str(), R""({"redopubbufsize":16385})"", GRD_DB_OPEN_CREATE, &dbVal);
}
...
void DbOpenFuzz(const uint8_t *data, size_t size)
{
    ...
    GRD_DBOpenInner(dbFileVal, configStrFlush, GRD_DB_OPEN_CREATE, &dbVal);
    ...
    GRD_DBOpenInner(dbFileVal, configStrBs, GRD_DB_OPEN_CREATE, &dbVal);
    ...
    GRD_DBOpenInner(dbFileVal, configStrMcn, GRD_DB_OPEN_CREATE, &dbVal);
    ...
    GRD_DBOpenInner(dbFileVal, configStrAll, GRD_DB_OPEN_CREATE, &dbVal);

    std::string fieldLowNumber = "{\"bufferPoolSize\": 102}";
    const char *configStrLowNumber = fieldLowNumber.data();
    GRD_DBOpenInner(dbFileVal, configStrLowNumber, GRD_DB_OPEN_CREATE, &dbVal);

    GRD_DBOpenInner(nullptr, configStrVal, GRD_DB_OPEN_CREATE, &dbVal);
    GRD_DBOpenInner(dbFileVal, configStrVal, GRD_DB_OPEN_CREATE, &dbVal);
...
void DbCloseResultSetFuzz()
{
    ...
    GRD_DBOpenInner(TEST_DB_FILE, CONFIG_STR, GRD_DB_OPEN_CREATE, &db2);
...
void TestGrdDbApGrdGetItem002Fuzz()
{
    ...
    GRD_DB *db = nullptr;
    GRD_DBOpenInner(TEST_DB_FILE, config, GRD_DB_OPEN_CREATE, &db);
