...
- 错误处理：innerkitsimpl 返回 `Status` enum；distributeddb 返回 `int` errno；跨层调用必须用 `StoreUtil::ConvertStatus` 转换。
...
| Device Manager | 设备管理 | 平台提供 | 是 | dms_service_enable=false 时禁用同步 |
